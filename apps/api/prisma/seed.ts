...
async function main() {
  ...
  const demoAdminEmail = (process.env.DEMO_ADMIN_EMAIL || 'service@autohaus-herrmann.de').trim();
  const demoAdminPassword = (process.env.DEMO_ADMIN_PASSWORD || 'fahrzeug123').trim();
