...
const BLOG_STATIC_RELATIVE = 'apps/web/public/Webseite Autohaus Herrmann/pages/blog';
...
export class BlogService {
  ...
  async generateCoverImage(dto: GenerateCoverImageDto, actor?: AuthUser) {
    ...
    const requestImage = async (requestPayload: Record<string, unknown>) => {
  ...
  private isBlogAsset(value: string) {
    return (
      value.includes('/public/blog/cover/') || value.includes('/public/blog/assets/')
    );
  }
  ...
  private async generateStaticPage(post: {
    ...
    const safeExcerpt =
      this.escapeHtml(post.excerpt?.trim() || 'Aktueller Beitrag aus dem Autohaus Herrmann.');
    ...
    const htmlContent =
      (post.content?.trim() && marked.parse(post.content)) || '<p>Kein Inhalt vorhanden.</p>';

    const targetDir = await this.resolveStaticDir();
    const filePath = path.join(targetDir, `${post.slug}.html`);

    const parsedContent = await Promise.resolve(marked.parse(post.content || ''));
