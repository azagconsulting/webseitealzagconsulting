...
export class DriveController {
  ...
  @Get('customers/root')
  getCustomersRoot(@CurrentUser() user?: AuthUser) {
    return this.driveService.getCustomersRootFolder(user);
  }

  @Get('customers/:customerId/folder')
  getCustomerFolder(
    @Param('customerId') customerId: string,
    @CurrentUser() user?: AuthUser,
  ) {
    return this.driveService.getCustomerFolder(customerId, user);
  }

  @Post('customers/:customerId/files')
  @UseInterceptors(
    FileInterceptor('file', {
      storage: memoryStorage(),
      limits: { fileSize: 25 * 1024 * 1024 },
  ...
  uploadCustomerFile(
    @Param('customerId') customerId: string,
    @Body() dto: UploadCustomerDriveFileDto,
    @UploadedFile() file: Express.Multer.File | undefined,
    @CurrentUser() user?: AuthUser,
  ) {
    if (!file) {
    ...
    return this.driveService.uploadCustomerFile(customerId, dto, file, user);
  }
