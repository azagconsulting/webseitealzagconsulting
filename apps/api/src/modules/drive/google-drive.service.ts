...
type GoogleTokenResponseWithAccess = Omit<GoogleTokenResponse, 'access_token'> & {
  access_token: string;
};
...
export class GoogleDriveService {
  ...
  private getDriveConfig() {
    ...
    if (!clientId || !clientSecret) {
      throw new BadRequestException('Google Drive OAuth ist nicht konfiguriert.');
    }
  ...
  private signState(payload: { tenantId: string; userId: string; returnTo: string }) {
  ...
  private async getUserInfo(accessToken: string) {
    ...
    await this.ensureOk(response, 'Google-Kontoinformationen konnten nicht geladen werden.');
  ...
  async createAuthUrl(returnTo?: string | null) {
    const tenantId = this.requireTenantId();
    const userId = this.requireUserId();
    const config = this.getDriveConfig();
    const redirectTarget = this.sanitizeReturnTo(
      returnTo,
      '/drive?tab=google',
    );
  ...
  async handleOAuthCallback(code: string, state: string) {
    ...
    const existing = await this.prisma.googleDriveConnection.findUnique({
      where: { tenantId_userId: { tenantId: parsed.tenantId, userId: parsed.userId } },
    });
  ...
  async disconnect() {
    ...
    await this.prisma.googleDriveConnection.delete({ where: { id: connection.id } });
    return { disconnected: true };
  }
  ...
  async listSharedDrives(): Promise<GoogleDriveSharedDrive[]> {
    ...
    await this.ensureOk(response, 'Shared Drives konnten nicht geladen werden.');
  ...
  async listFiles(dto: ListGoogleDriveFilesDto): Promise<GoogleDriveFileListResponse> {
    const { accessToken } = await this.getValidAccessToken();
    const pageSize = Math.min(dto.pageSize ?? 20, 100);
    const queryParts = ["trashed = false", "mimeType != 'application/vnd.google-apps.folder'"];
    if (dto.search?.trim()) {
      const cleaned = dto.search.trim().replace(/\\/g, '\\\\').replace(/'/g, "\\'");
      queryParts.push(`name contains '${cleaned}'`);
    }
    const params = new URLSearchParams();
    params.set('pageSize', String(pageSize));
    params.set('fields', 'nextPageToken, files(id,name,mimeType,size,modifiedTime,driveId,webViewLink)');
    ...
    await this.ensureOk(response, 'Google Drive Dateien konnten nicht geladen werden.');
