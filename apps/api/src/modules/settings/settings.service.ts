...
const CHATBOT_OPENAI_SETTING_KEY = 'chatbot-openai-settings';
...
export class SettingsService {
  ...
  async getChatbotLauncher(): Promise<{ enabled: boolean; updatedAt?: string } | null> {
    const tenantId = this.context.getTenantId();
    const userId = this.context.getUserId();
    if (!tenantId || !userId) {
      return null;
  ...
  private async getGlobalStoredSmtpSettings(
    tenantId?: string,
  ): Promise<{
    primary: SmtpCredentials;
    contactForm?:
