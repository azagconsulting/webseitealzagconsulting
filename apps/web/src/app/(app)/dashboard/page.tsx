...
import { CalendarDays, Clock3, Globe2, Loader2, Mail, MousePointerClick, RefreshCw, TrendingUp } from "lucide-react";
...
type AppointmentSlotStatus = "free" | "blocked";

interface DashboardAppointmentSlot {
  id: string;
  date: string;
  start: string;
  end: string;
...
interface TermineSlotsResponse {
  items?: DashboardAppointmentSlot[];
}
...
export default function DashboardPage() {
  ...
  const [appointmentSlots, setAppointmentSlots] = useState<DashboardAppointmentSlot[]>([]);
  const [appointmentsLoading, setAppointmentsLoading] = useState(false);
  const [appointmentsError, setAppointmentsError] = useState<string | null>(null);
  const [appointmentDetailsOpen, setAppointmentDetailsOpen] = useState(false);
  const [selectedAppointment, setSelectedAppointment] = useState<DashboardAppointmentSlot | null>(null);
  ...
  const loadAppointments = useCallback(
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        const to = toIsoDate(addDays(today, 90));
        const params = new URLSearchParams({ from, to });
        const response = await authorizedRequest<TermineSlotsResponse>(`/termine/slots?${params.toString()}`, {
          signal,
        });
        setAppointmentSlots(Array.isArray(response?.items) ? response.items : []);
      } catch (err) {
        if (signal?.aborted) return;
        setAppointmentsError(err instanceof Error ? err.message : "Termine konnten nicht geladen werden.");
      } finally {
        if (!signal?.aborted) {
  ...
  const recentBookedCount = useMemo(() => {
    const now = new Date();
    const sevenDaysAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    return appointmentSlots.reduce((count, slot) => {
      if (!slot.bookedAt) return count;
      const bookedAt = new Date(slot.bookedAt);
      if (Number.isNaN(bookedAt.getTime())) return count;
      return bookedAt >= sevenDaysAgo && bookedAt <= now ? count + 1 : count;
    }, 0);
  }, [appointmentSlots]);

  const nextAppointment = useMemo(() => {
    const now = new Date();
    const byStart = (a: DashboardAppointmentSlot, b: DashboardAppointmentSlot) => {
      const dateA = parseSlotStartDate(a);
      const dateB = parseSlotStartDate(b);
      if (!dateA || !dateB) return 0;
      return dateA.getTime() - dateB.getTime();
    };

    const upcomingOwn = appointmentSlots
      .filter((slot) => slot.bookedById === user?.id && isBookedSlot(slot))
      .filter((slot) => {
        const start = parseSlotStartDate(slot);
        return start ? start >= now : false;
      })
      .sort(byStart);

    return upcomingOwn[0] ?? null;
  }, [appointmentSlots, user?.id]);

  const openAppointmentDetails = useCallback((slot: DashboardAppointmentSlot) => {
  ...
  return (
    <section className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card
          title="Termine"
          description="Neu gebuchte Termine und Ihr nächster Termin."
          ...
          action={(
            ...
            >
              {appointmentsLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
            </Button>
          )}
        >
          {appointmentsError ? <p className="text-xs text-rose-300">{appointmentsError}</p> : null}
          {appointmentsLoading && appointmentSlots.length === 0 ? (
            <p className="flex items-center gap-2 text-sm text-slate-300">
          ...
          <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Neu Gebucht (7 Tage)</p>
              <div className="mt-3 flex items-center gap-3">
                <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-white/10 text-white">
                  <CalendarDays className="h-5 w-5" />
                </span>
                <div>
                  <p className="text-2xl font-semibold leading-tight text-white">{formatNumber(recentBookedCount)}</p>
                  <p className="text-xs text-slate-400">Buchungen der letzten 7 Tage</p>
                </div>
              </div>
            </div>

            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Ihr Nächster Termin</p>
              {nextAppointment ? (
                <button
                  type="button"
                  onClick={() => openAppointmentDetails(nextAppointment)}
                  className="mt-3 w-full rounded-xl border border-white/10 bg-white/5 p-3 text-left transition hover:border-white/25 hover:bg-white/10"
                >
                  <p className="text-base font-semibold text-white">{nextAppointment.title}</p>
                  <p className="mt-1 text-sm text-slate-200">
                    {formatAppointmentDate(nextAppointment.date)} • {nextAppointment.start} - {nextAppointment.end} Uhr
                  </p>
                  <p className="mt-1 text-xs text-slate-400">
                    {nextAppointment.customerName || nextAppointment.attendeeName || "Teilnehmer noch offen"}
                  </p>
                  <p className="mt-2 text-[11px] text-sky-300">Termindetails öffnen</p>
                </button>
              ) : (
                <p className="mt-3 text-sm text-slate-400">Kein kommender persönlicher Termin vorhanden.</p>
              )}
            </div>
          </div>

          <div className="mt-4">
            <Button size="sm" variant="ghost" onClick={() => router.push("/workspace/termine")}>
        ...
        {selectedAppointment ? (
          <div className="space-y-4 text-sm text-slate-200">
            <div className="rounded-xl border border-white/10 bg-white/5 p-3">
              <p className="text-xs uppercase tracking-[0.25em] text-slate-400">Termin</p>
              <p className="mt-1 text-base font-semibold text-white">{selectedAppointment.title}</p>
              <p className="mt-1 text-sm text-slate-300">
                {formatAppointmentDate(selectedAppointment.date)} • {selectedAppointment.start} - {selectedAppointment.end} Uhr
              </p>
            </div>

            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                <p className="text-xs uppercase tracking-[0.25em] text-slate-400">Teilnehmer</p>
                <p className="mt-1 text-sm text-white">
                  {selectedAppointment.customerName || selectedAppointment.attendeeName || "Nicht hinterlegt"}
                </p>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                <p className="text-xs uppercase tracking-[0.25em] text-slate-400">Status</p>
                <p className="mt-1 text-sm text-white">
                  {selectedAppointment.bookedAt || selectedAppointment.customerId || selectedAppointment.attendeeName
                    ? "Gebucht"
            ...
            {(selectedAppointment.attendeeEmail || selectedAppointment.attendeePhone) ? (
              <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                <p className="text-xs uppercase tracking-[0.25em] text-slate-400">Kontakt</p>
                {selectedAppointment.attendeeEmail ? (
                  <p className="mt-1 text-sm text-white">{selectedAppointment.attendeeEmail}</p>
                ) : null}
                {selectedAppointment.attendeePhone ? (
                  <p className="text-sm text-white">{selectedAppointment.attendeePhone}</p>
                ) : null}
              </div>
            ) : null}
            ...
            {selectedAppointment.meetingLink ? (
              <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                <p className="text-xs uppercase tracking-[0.25em] text-slate-400">Meeting</p>
                <Button
                  size="sm"
                  className="mt-2"
            ...
            {selectedAppointment.bookingNotes ? (
              <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                <p className="text-xs uppercase tracking-[0.25em] text-slate-400">Notizen</p>
                <p className="mt-1 whitespace-pre-wrap text-sm text-white">{selectedAppointment.bookingNotes}</p>
              </div>
            ) : null}

            {selectedAppointment.bookedAt ? (
              <p className="text-xs text-slate-400">
                Gebucht am {formatDateTime(selectedAppointment.bookedAt)}
              </p>
            ) : null}
...
function parseSlotStartDate(slot: Pick<DashboardAppointmentSlot, "date" | "start">) {
...
function isBookedSlot(slot: Pick<DashboardAppointmentSlot, "bookedAt" | "customerId" | "attendeeName">) {
  return Boolean(slot.bookedAt || slot.customerId || slot.attendeeName);
}
