  ...
  const fetchMailboxData = useCallback(async (customerId?: string | null) => {
    ...
    try {
      ...
      if (customerId) {
        ...
        const contactSources = new Set(["contact-form"]);
        const contactRequestLeads = leadsResponse.filter((lead) => contactSources.has(lead.source ?? ""));
        const regularLeads = leadsResponse.filter((lead) => !contactSources.has(lead.source ?? ""));
        const contactUnread = contactRequestLeads.filter((lead) => !lead.processedAt).length;
        ...
        setContactRequests(contactRequestLeads);
