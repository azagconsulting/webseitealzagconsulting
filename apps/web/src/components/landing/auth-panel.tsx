...
const CUSTOMER_HOME_PATH = "/kundenportal";

const resolveTargetPath = (role?: string | null) =>
  role === "CUSTOMER" ? CUSTOMER_HOME_PATH : APP_HOME_PATH;
...
export function LandingAuthPanel() {
  ...
  const { login, verifyLoginCode, error, user } = useAuth();
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const targetPath = resolveTargetPath(result.user.role);
      setStatus("success");
  ...
  const handleVerify = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const authedUser = await verifyLoginCode({
        email: form.email,
        code,
        deviceId: challenge?.deviceId,
      });
      const targetPath = resolveTargetPath(authedUser.role);
      setStatus("success");
      router.push(targetPath);
      if (typeof window !== "undefined") {
        window.top?.location.assign(targetPath);
    ...
    <div id="zugang" className="rounded-[32px] border border-white/10 bg-white/5 p-6 shadow-[0_30px_70px_rgba(8,15,40,0.4)]">
      <div className="space-y-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            Bitte melde dich mit deinen persönlichen Zugangsdaten an, um deinen Bereich zu öffnen.
          </p>
        </div>
      </div>
      ...
      {user && (
        <p className="mt-4 rounded-2xl border border-emerald-300/40 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-200">
          Bereits eingeloggt als <span className="font-semibold">{user.email}</span>. Gehe direkt zu{" "}
          {user.role === "CUSTOMER" ? "deinem Kundenbereich" : "deinem Dashboard"}.
        </p>
      )}
