...
export function useChatbotOpenAiKey() {
  ...
  const fetchKey = useCallback(async () => {
    setLoading(true);
    try {
      const response = await authorizedRequest<OpenAiSettingsResponse | null>("/settings/chatbot/openai");
  ...
  const saveKey = useCallback(
    async (nextKey: string | null) => {
      setSaving(true);
      try {
        await authorizedRequest<OpenAiSettingsResponse>("/settings/chatbot/openai", {
          method: "PUT",
