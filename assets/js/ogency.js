(function ($) {
  ...
  if ($(".mobile-nav__container .main-menu__list").length) {
    ...
    dropdownAnchor.each(function (index) {
      let self = $(this);
      let subMenu = self.parent().children("ul").first();
      let subMenuId = "mobile-submenu-" + (index + 1);
      let toggleBtn = document.createElement("BUTTON");
      toggleBtn.setAttribute("type", "button");
      toggleBtn.setAttribute("aria-label", "Untermenü umschalten");
      ...
      self.find("button").on("click", function (e) {
        e.preventDefault();
        let self = $(this);
        let parentLink = self.parent();
        let menu = parentLink.parent().children("ul").first();
        let isExpanded = !self.hasClass("expanded");
        self.toggleClass("expanded");
        self.attr("aria-expanded", isExpanded ? "true" : "false");
        parentLink.toggleClass("expanded");
        if (menu.length) {
  ...
  let mobileNavWrapper = $(".mobile-nav__wrapper");
  let mobileNavOpeners = $(".main-menu__toggler.mobile-nav__toggler");
  function setMobileNavState(isOpen) {
    if (!mobileNavWrapper.length) {
      return;
    }
    mobileNavWrapper.toggleClass("expanded", isOpen);
    mobileNavWrapper.attr("aria-hidden", isOpen ? "false" : "true");
    if (isOpen) {
      mobileNavWrapper.removeAttr("inert");
    } else {
      mobileNavWrapper.attr("inert", "");
    }
    $("body").toggleClass("locked", isOpen);
    mobileNavOpeners.attr("aria-expanded", isOpen ? "true" : "false");
  }
  ...
  if ($(".mobile-nav__toggler").length) {
    $(".mobile-nav__toggler").on("click", function (e) {
      e.preventDefault();
      let isOpening = !mobileNavWrapper.hasClass("expanded");
      setMobileNavState(isOpening);
    });
  }

  $(document).on("keydown", function (e) {
    if (e.key === "Escape" && mobileNavWrapper.hasClass("expanded")) {
      setMobileNavState(false);
    }
  });
  ...
  if ($(".search-toggler").length) {
    $(".search-toggler").on("click", function (e) {
      ...
      if (mobileNavWrapper.length) {
        ...
        mobileNavWrapper.attr("aria-hidden", "true");
        mobileNavWrapper.attr("inert", "");
        mobileNavOpeners.attr("aria-expanded", "false");
      }
  ...
  if ($(".faq-page__accrodion").length) {
    ...
    accrodionGrp.each(function () {
      ...
      function setAccordionState(item, isOpen, animate) {
        var $item = $(item);
        var $title = $item.find(".accrodion-title").first();
        var $content = $item.find(".accrodion-content").first();
      ...
      accordion.each(function (index) {
        ...
        var $title = $item.find(".accrodion-title").first();
        var $content = $item.find(".accrodion-content").first();
        var titleId = accrodionName + "-title-" + (index + 1);
        var contentId = accrodionName + "-content-" + (index + 1);
        var isInitiallyOpen = $item.hasClass("active");
        ...
        $content.attr({
          ...
          "aria-hidden": isInitiallyOpen ? "false" : "true"
        });

        setAccordionState($item, isInitiallyOpen, false);
        ...
        $title.on("click", function () {
          ...
          var $currentGroup = $(".faq-page__accrodion." + accrodionName);

          if ($currentItem.hasClass("active")) {
            setAccordionState($currentItem, false, true);
            return;
          }

          $currentGroup.find(".accrodion").each(function () {
            setAccordionState($(this), false, true);
          });

          setAccordionState($currentItem, true, true);
        });
      });
    });
  }
  ...
  var lastScrollTop = Math.max($(window).scrollTop(), 0);
  function updateStickyHeaderState() {
    if ($(".stricked-menu").length) {
      var headerScrollPos = 130;
      var stricky = $(".stricked-menu");
      var currentScrollTop = Math.max($(window).scrollTop(), 0);
      var isScrollingUp = currentScrollTop < lastScrollTop;
      var shouldShow = currentScrollTop > headerScrollPos && isScrollingUp;

      if (shouldShow) {
        stricky.addClass("stricky-fixed");
        stricky.attr("aria-hidden", "false");
        stricky.removeAttr("inert");
      } else {
        stricky.removeClass("stricky-fixed");
        stricky.attr("aria-hidden", "true");
        stricky.attr("inert", "");
      }
